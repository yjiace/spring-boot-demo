# Spring Boot Demo 项目 AI 编程规则（项目规则）
...
- **过滤器**: `JwtTokenFilter` - 负责 Token 验证和用户认证
- **配置类**: `SecurityConfig`、`JwtConfig` - 已配置完成,不要修改
- **工具类**: `UserUtil` - 提供用户信息获取和数据填充

### JWT 配置规则

**禁止**在代码中硬编码 JWT 配置,必须使用 `application.yaml` 中的 `jwt` 配置:
- `jwt.enable`: 控制鉴权开关
- `jwt.sso`: 控制单点登录
- `jwt.tokenHead`: Token 请求头名称
- `jwt.anonUrl`: 匿名访问路径白名单
- `jwt.authUrl`: 需登录但不校验权限的路径
- `jwt.noAccess`: 禁止访问的路径黑名单
...
**规则**:
- 需要匿名访问的接口路径,必须添加到 `jwt.anonUrl` 配置中
- 需要登录但不校验权限的接口,必须添加到 `jwt.authUrl` 配置中
- 需要权限控制的接口,依赖 Redis 中的 `authorities` 字段自动校验
- **禁止**在 Controller 中手动校验 Token 或权限

#### 2. 获取当前用户信息

**必须**使用 `UserUtil` 工具类获取当前登录用户信息:
...
// 获取当前 Token
String token = UserUtil.getCurrentUserToken();
```

**禁止**:
- 从 `SecurityContextHolder` 直接获取
- 从 `HttpServletRequest` 手动解析 Token
- 在 Service 层传递 userId 参数(应使用 `UserUtil.getCurrentAuditor()`)

#### 3. JPA 审计字段自动填充

**规则**:
- Entity 必须继承 `BaseEntity`,自动包含审计字段(`createdBy`、`createdTime`、`updatedBy`、`updatedTime`)
- **禁止**手动设置审计字段,JPA 会通过 `SpringSecurityAuditorAware` 自动填充
- `createdBy` 和 `updatedBy` 自动填充当前登录用户 ID(通过 `UserUtil.getCurrentAuditor()`)

#### 4. 用户姓名填充规则

**调用时机**: 当返回的数据需要显示创建人姓名(`createdName`)、修改人姓名(`updatedName`)或其他用户姓名字段时,调用 `UserUtil.setUserName()` 进行填充

**调用位置**: 可以在任何位置调用(Controller、Service、工具类等),通常在数据返回前调用

**标准用法**:
```java
// 分页查询
@GetMapping("/page")
public Page<UserResponse> page(Pageable pageable, HttpServletRequest request) {
    Page<User> page = userService.findAll(WebUtils.getParametersStartingWith(request, "search_"), pageable);
    return UserUtil.setUserName(page.map(UserMapper.INSTANCE::toResponse));
}
...
**Response DTO 要求**:
- 如果需要显示用户姓名,必须包含对应的姓名字段(如 `createdName`、`updatedName`)
- 姓名字段类型必须为 `String`
- 如果有自定义用户 ID 字段,必须同时定义对应的姓名字段(如 `managerId` 对应 `managerName`)
...
> ⚠️ **重要提示**：
> - **直接创建 Java 文件即可**，父目录会自动生成，无需手动创建目录
> - **禁止**执行 `mkdir`、`New-Item` 等目录创建命令（会导致用户确认，中断自动化流程）
> - **禁止**创建 `.gitkeep` 等占位文件
> - 如果目录不存在，创建文件时会自动生成对应的包目录
...
**注意：本接口文档面向的是 AI 编程软件，旨在告诉 AI 接口的格式，而不是面向开发者。**

### 步骤 10: 更新 SQL 文件
添加业务时，需要在项目根目录的 sql 文件中添加对应业务的 sql 语句。

### 步骤 11: 代码自检与质量评估（必须执行！）

完成上述所有步骤后，**必须**进行以下自检流程：
...
| 检查项 | 检查内容 |
|--------|----------|
| **Entity** | 是否继承 `BaseEntity`、是否添加 `@Entity`/`@Table`/`@SQLRestriction` 注解、字段是否有中文注释 |
| **Repository** | 是否继承 `BaseRepository<实体, 主键类型>` |
| **Service** | 是否继承 `BaseService`、是否添加 `@Service`/`@Slf4j`/`@Transactional(readOnly = true)` |
| **DTO** | 是否实现 `Serializable`、是否有 `serialVersionUID`、时间字段是否添加 `@JsonFormat`、字段是否有中文注释 |
| **Mapper** | 是否使用 `@Mapper` 注解并设置 `nullValuePropertyMappingStrategy`、是否定义 `INSTANCE`、是否包含 `toResponse` 和 `init` 方法 |
| **Controller** | 是否添加 `@ResponseSysResult`、是否使用 `UserUtil.setUserName()` 填充用户名、方法是否有中文注释 |
| **命名规范** | 类名、方法名、变量名是否符合命名规范 |
| **注释规范** | 类注释是否包含 `@author` 和 `@date` |
...
| 维度 | 分值 | 评分标准 |
|------|------|----------|
| **完整性** | 30 分 | 所有必需文件是否创建完成（每缺少一项扣 3 分） |
| **规范符合度** | 30 分 | 代码是否严格遵循本规则文件（每处不符合扣 2 分） |
| **代码质量** | 20 分 | 代码结构清晰、无重复代码、合理封装（每处问题扣 2 分） |
| **注释完整度** | 10 分 | 类/方法/字段注释是否完整（每缺少一处扣 1 分） |
| **一致性** | 10 分 | 与参考示例（User 模块）保持一致（每处差异扣 1 分） |

**评分等级：**
- **90-100 分**：优秀 ✅ 可直接提交
- **80-89 分**：良好 ⚠️ 建议优化后提交
- **70-79 分**：合格 ⚠️ 需要修改后再评估
- **70 分以下**：不合格 ❌ 必须修改后重新自检

#### 11.4 生成修改意见

如果评分低于 90 分，**必须**列出具体修改意见：
